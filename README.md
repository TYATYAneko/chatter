...
- **グループ参加**: コードを入力して参加
- **グループ退出**: メニューから退出可能
- **グループ情報**: メンバー一覧を確認、作成者はメンバーを退出させることが可能

### ノート共有
- **メモ投稿**: テキストメモの投稿・共有
- **画像送信**: 画像ファイルの送信・共有（5MBまで、Base64形式）
- **リプライ機能**: メッセージに返信、引用表示でスレッド形式の会話が可能
...
### v2.11.1
- **UI改善**
  - バージョン表示をロビー画面のタイトル横に移動

### v2.11.0
- **ローディング画面追加**
  - 処理中にスピナー付きのローディングオーバーレイを表示
  - ダークテーマ対応

### v2.10.1
- **バグ修正**
  - PCでリプライボタンが機能しない問題を修正（グローバル関数のエクスポート追加）
  - メッセージの並び順が乱れる問題を修正（ソート処理をlocaleCompareから純粋な文字列比較に変更）

### v2.10.0
- **リプライ機能**
  - メッセージに返信（リプライ）機能を追加
  - メッセージにマウスホバーで返信ボタンを表示
  - 返信先のメッセージを引用表示
  - 引用をクリックすると元のメッセージにスクロール

### v2.9.0
- **グループ情報画面の改善**
  - メンバー表示を箇条書き（リスト形式）に変更
  - 作成者と自分を識別するバッジを追加
- **メンバー管理機能**
  - グループ作成者がメンバーを退出させる機能を追加
  - 無効なメンバー（アカウント削除済み）の検出機能
  - 退出させられたユーザーがグループにアクセスした際の自動クリーンアップ
